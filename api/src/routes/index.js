...
router.get ('/expedientes', async (req,res) => { // llama todos los expedientes en la DB
    ...
    if(numexpediente) {
        const expedientesnum = await infoTotal.filter(p => p.numexpediente.includes(numexpediente))
...
router.get("/expedientes/:numexpediente", async (req, res) => {    
    ...
    Expediente.findOne({where: {numexpediente:req.params.numexpediente}})
    .then (p =>{
        res.json (p)
    })   
...
router.get("/tickets/:id", async (req, res) => {    
    ...
    Ticket.findOne({where: {numticket:numticket}})
    .then (p =>{
        res.json (p)
    })
});
